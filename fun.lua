...
{{text = '⌁ قناة السورس ⚡',url="t.me/"..IADFI..""}},
}
...
return bot.sendText(msg_chat_id,msg_id,"*لا استطيع استخدام الأمر علي توم*","md",true)  
end
...
if tonumber(Message_Reply.sender_id.user_id) == tonumber(1427981991) then
return bot.sendText(msg_chat_id,msg_id,"*لا استطيع استخدام الأمر علي توم بيه*","md",true)  
end
if tonumber(Message_Reply.sender_id.user_id) == tonumber(Fast) then
return bot.sendText(msg_chat_id,msg_id,"*لا استخدم الأمر على نفسي يستا 😂*","md",true)  
end
if ban.first_name then
...
if tonumber(Message_Reply.sender_id.user_id) == tonumber(1427981991) then
return bot.sendText(msg_chat_id,msg_id,"*لا استطيع استخدام الأمر على توم*","md",true)  
end
if tonumber(Message_Reply.sender_id.user_id) == tonumber(Fast) then
return bot.sendText(msg_chat_id,msg_id,"*لا استخدم الأمر على نفسي يستا 😂*","md",true)  
end
...
keyboard = {} 
keyboard.inline_keyboard = {
{{text = '⌁ قناة السورس ⚡',url='http://t.me/'..IADFI..''}},
}
...
{{text=" "..FiJABWA.." ",url=("t.me/"..UserInfo.username or "t.me/Sourse1_milano")}},
}}
...
if text == "hdryu" or text == "vhiii" then 
if ChannelJoinch(msg) == false then
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = Redis:get(Fast..'Chat:Channel:Join:Name'..msg.chat_id), url = 't.me/'..Redis:get(Fast..'Chat:Channel:Join'..msg.chat_id)}, },}}
return send(msg.chat_id,msg.id,'*\n⌔︙عليك الاشتراك في قناة البوت لأستخدام الاوامر*',"md",false, false, false, false, reply_markup)
end
...
local datar = {data = {{text = "قناة السورس" , url = 'http://t.me/'..IADFI..''}}}
if #json < 5 then
for i = 1,#json do
title = json[i].title
...
bot.sendAudio(msg_chat_id,msg_id,'./intk'..rand..'.mp3',tostring(inoi),"html",nil,tostring(inoi),"@'..IADFI..'")
sleep(1)
